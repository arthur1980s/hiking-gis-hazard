...
## 部署记录
- **2026-08-16 v21** (7e9addf): 移除失效 NASA GIBS 瓦片图层(旧 PNG 图层被 NASA 下线全 400, 新 MVT 为 epsg4326 网格无法直用于 WebMercator) → 山火统一走 FIRMS 点数据(超时放宽 12s); CSP script-src 放行 static.cloudflareinsights.com(修 beacon violation), img-src 移除 gibs.earthdata.nasa.gov。nginx 配置同步更新。版本号三处同步 v21。
